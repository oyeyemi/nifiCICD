...
# The DN string NiFi will extract from the client certificate.
#
# IMPORTANT: openssl's "-subj" flag and Java's X500Principal string form
# list RDNs in OPPOSITE order. "-subj" below is written CN first, ...,
# C last (as openssl expects), but NiFi/Java reports the identity as
# C first, ..., CN last. This has been confirmed against a live NiFi
# nifi-user.log entry - do not "fix" this back to CN-first without
# re-checking against a real log line, since it's easy to get backwards.
# If login still fails with "Insufficient Permissions", the ground truth
# is always: docker compose exec nifi grep -i identity logs/nifi-user.log
...
# nifi.sensitive.props.key encrypts sensitive processor properties (e.g.
# passwords typed into a flow). It's independent of the TLS certs, so if
# an existing one is found, preserve it across --force cert regeneration -
# losing it would make anything already encrypted with it unreadable.
...
# Reuse the existing sensitive props key if one was found; otherwise NiFi
# requires at least 12 characters, so generate a longer one for margin.
...
# Preserve any USER_*_P12_PASSWORD lines add-user.sh may have already
# added, since the .env rewrite below would otherwise silently drop them.
EXISTING_USER_PASSWORDS=""
if [[ -f "${ROOT_DIR}/.env" ]]; then
  EXISTING_USER_PASSWORDS="$(grep '^USER_.*_P12_PASSWORD=' "${ROOT_DIR}/.env" 2>/dev/null || true)"
fi
...
if [[ -n "${EXISTING_USER_PASSWORDS}" ]]; then
  echo "${EXISTING_USER_PASSWORDS}" >> "${ROOT_DIR}/.env"
fi
